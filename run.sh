...
function proxify_usage {
    ...
    echo "usage: run.sh [backend_hostname_or_ip:port] [ssl]"
    echo "   ex: run.sh localhost:6379"
    echo "   ex: run.sh 8.8.8.8 [ssl]"
    echo ""
...
function proxify_main {
    if [[ $2 = "ssl" ]]
    then
       BACKEND_PROTO="ssl\ verify\ none"
    else
       BACKEND_PROTO=""
    fi

    echo "Proxying $1 to http://localhost and https://localhost"
    eval $(which docker) run -e "BACKEND=$1" -e "BACKEND_PROTO=$BACKEND_PROTO" -it -p 7080:7080 -p 80:80 -p 443:443 brennents/proxify:latest
